...
public class Segment
{
    ...
    public static int lenSqr(Point a, Point b)
    {
    ...
    public Point IntersectionLine(Segment s)
    {
    ...
    {
        int t = (p1.x - s.p1.x) * (s.p1.y - s.p2.y) - (p1.y - s.p1.y) * (s.p1.x - s.p2.x);
        int u = (p1.x - s.p1.x) * (p1.y - p2.y) - (p1.y - s.p1.y) * (p1.x - p2.x);
        int d = (p1.x - p2.x) * (s.p1.y - s.p2.y) - (p1.y - p2.y) * (s.p1.x - s.p2.x);

        if (d == 0 || (d < 0 && (t < d || u < d || t > 0 || u > 0)) || (d > 0 && (t > d || u > d || t < 0 || u < 0)))
            return null;
